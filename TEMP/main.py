...
flags.DEFINE_string("dataset", "single", "The name of dataset [celebA, mnist, lsun]")
flags.DEFINE_string("checkpoint_dir", "checkpoint_single", "Directory name to save the checkpoints [checkpoint]")
flags.DEFINE_string("sample_dir", "samples_single", "Directory name to save the image samples [samples]")
...
def main(_):
    ...
    with tf.device("/gpu:0"):
        ...
        sample_z = tf.placeholder(tf.float32, [FLAGS.sample_size, z_dim], name='sample_z_noise')
        ...
        net_g2, g2_logits = generator_simplified_api(sample_z, is_train=False, reuse=True)
    ...
    for epoch in range(FLAGS.epoch):
        ...
        # sample_images = np.ndarray([FLAGS.sample_size, FLAGS.output_size, FLAGS.output_size, FLAGS.c_dim]).astype(np.float32)
        # for i in range(FLAGS.batch_size):
        #     sample_images[i, :, :, 0] = sample[i]
        ...
        for idx in xrange(0, batch_idxs):
            ...
            # batch_images = np.ndarray([FLAGS.batch_size, FLAGS.output_size, FLAGS.output_size, FLAGS.c_dim]).astype(np.float32)
            # for i in range(FLAGS.batch_size):
            #     batch_images[i,:,:,0]=batch[i]
            batch_images = np.array(batch).astype(np.float32)
            batch_z = np.random.normal(loc=0.0, scale=1.0, size=(FLAGS.batch_size, z_dim)).astype(np.float32)  # batch_z = np.random.uniform(low=-1, high=1, size=(FLAGS.batch_size, z_dim)).astype(np.float32)
            ...
            if np.mod(iter_counter, FLAGS.sample_step) == 0:
                ...
                img= sess.run(net_g2.outputs,feed_dict={sample_z: sample_seed})
