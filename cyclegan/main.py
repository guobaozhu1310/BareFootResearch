...
img_width = 256
img_layers = 3
img_sizes = img_height * img_width
...
class CycleGAN():
    ...
    def input_setup(self):
        ...
        filenames_A = tf.train.match_filenames_once("./datasets/foot/org_left/*.jpg")
        self.queue_length_A = tf.size(filenames_A)
        filenames_B = tf.train.match_filenames_once("./datasets/foot/new_left/*.jpg")
    ...
    def input_read(self, sess):
        ...
        for i in range(max_images): 
            ...
            # tempa=tf.size(image_tensor)
            # tempb=img_sizes*batch_size*img_layers
            # if(tempa == tempb):
            self.A_input[i] = image_tensor.reshape((batch_size,img_height, img_width, img_layers))
        ...
        for i in range(max_images):
            ...
            # tempb = img_sizes * batch_size * img_layers
            # if (tempa == tempb):
            self.B_input[i] = image_tensor.reshape((batch_size,img_height, img_width, img_layers))
